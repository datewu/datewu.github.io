...
[root@wutuofu ~]# vi /etc/yum.repos.d/openstack.repo
[root@wutuofu ~]# yum install -y   python2-openstackclient  openstack-neutron
[root@wutuofu shells]# cat source
...
```shell
[root@wutuofu shells]# cat vip.sh
. ./source
...
申请创建`port`
```shell
[root@wutuofu shells]# cat create-port.sh
#!/bin/bash
. ./source
network=your_network_id
...
```shell
[root@wutuofu shells]# cat allow-address-pairs-load-test.sh
. ./source
vm_port=vm_port_id
function setup
{
