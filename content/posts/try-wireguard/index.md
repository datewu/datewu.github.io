...
#!/bin/bash
sudo -i
[root@wutuofu ~]# cat /etc/redhat-release
Red Hat Enterprise Linux Server release 7.6 (Maipo)
[root@wutuofu ~]# echo "net.ipv4.ip_forward = 1" >> /etc/sysctl.conf
[root@wutuofu ~]# sysctl -p

### install packages
[root@wutuofu ~]#  curl -Lo /etc/yum.repos.d/wireguard.repo https://copr.fedorainfracloud.org/coprs/jdoss/wireguard/repo/epel-7/jdoss-wireguard-epel-7.repo
[root@wutuofu ~]#  yum  install -y  epel-release wireguard-dkms wireguard-tools
[root@wutuofu ~]#  yum  install -y  epel-release
[root@wutuofu ~]#  rpm -ivh https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
[root@wutuofu ~]#  yum update -y
[root@wutuofu ~]#  yum  install -y  epel-release wireguard-dkms wireguard-tools
[root@wutuofu ~]#  init 6
...
```shell
### start wg0 device
[root@wutuofu ~]# cat start-wireguard.sh
...
### server side
[root@wutuofu ~]#  yum install tcpdump
[root@wutuofu ~]#  tcpdump -i eth0 udp port 58855 -vv -X
```
